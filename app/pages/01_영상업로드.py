...
파일명: app/pages/01_영상업로드.py
 ...
 2개 영상(보행 정면·측면) 업로드 → 기본 메타(FPS/해상도/길이) 확인 →
...
st.caption("측면 촬영 필수 · 영상 길이 최소 20초 · 동작 반복 최소 3회 · 전신 프레임 유지 · 삼각대 고정 · 거리 3 m / 높이 1.2 m 권장")
...
if submitted:
    # 측면 필수 검증
    if gait_side is None:
        st.error("보행 측면 영상은 필수입니다.")
        st.stop()
    ...
    ordered_cols = ["클립","상태","파일","해상도","FPS","길이(초)","avg_vis","vis_ratio","occlusion","jitter_std","판정"]
    ...
    def fail_reasons(qm: dict) -> list[str]:
        ...
        if qm.get("duration_sec", 0) < th.get("duration_min", 20):       fails.append(f"영상 길이<{th.get('duration_min', 20)}s")
        return fails
    ...
    st.markdown("### 재촬영 가이드")
    guide_rows = []
    qm_side = q_results.get("gait_side", {})
    if qm_side and not qm_side.get("pass", False):
        miss = fail_reasons(qm_side)
        guide_rows.append({
            "클립": "gait_side",
            "재촬영 사유": ", ".join(miss),
            "가이드": "\n".join(guidance(miss))
    ...
    if guide_rows:
        ...
        st.write("보행 측면 클립은 재촬영 권고 항목이 없습니다.")
    ...
    # 다음 단계 활성 조건: 보행 측면 통과
    side_ok = q_results.get("gait_side", {}).get("pass", False)
    if side_ok:
        st.success("보행 측면 영상 통과. 다음 단계로 이동 가능합니다.")
        st.link_button("동작분석으로 이동", "02_동작분석")
    else:
        st.warning("보행 측면이 ‘재촬영’이면 동작분석 단계에서 제외됩니다.")
